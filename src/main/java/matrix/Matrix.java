...
public class Matrix {
	...
	public Matrix(double[][] array) {
		System.out.println("In Matrix Constructor");
	...
	private void compressWithK(int k){
		...
		for(int i = 0; i < k; i ++) {
			for(int j = 0; j < numRows; j ++) {
				retU[j][i] = S.get(i, i) * U.get(j, i);
			}
		}
		System.out.println("End First For Loop");
		this.U = retU.clone();
